...
class Meshes:
    def __init__(
        ...
        **kwargs,
        ...
        floor_height = h_data.min(axis=0).min(axis=0)[2]
        data = prepare_meshes(
            h_data,
...
def prepare_meshes(data, canonicalize=True, always_on_floor=False, floor_height=0):
    if canonicalize:
    ...
    # floor_height = data[..., 2].min(1)[:, None]
...
def prepare_obj_meshes(data, canonicalize=True, always_on_floor=False, floor_height=0):
    if canonicalize:
        print("No canonicalization for now")
