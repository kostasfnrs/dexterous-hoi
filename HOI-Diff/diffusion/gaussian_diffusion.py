...
class GaussianDiffusion:
    ...
    def __init__(self, conf: DiffusionConfig):
        ...
        self.guidance_time = conf.guidance_time
    ...
    def p_mean_variance(
        ...
        # model_output, contact_output = model_output[0], model_output[1]       #  NOTE modify here for contact
    ...
    def condition_mean(self, cond_fn, x, t, model_kwargs=None):
        """
    ...
    def p_sample(
        ...
        if cond_fn is not None:   
            ...
            # is_static =  model_kwargs['y']['afford_labels'][:,22:]
            # is_static = (is_static>0.7).int()

            # print(f"test: {is_static}")
            
            if perturb_last == True:
                if t[0]<1:
                    print(f" perturb the last step !! ")
                    out["mean"] = self.condition_mean(
                        cond_fn, out['mean'], t, model_kwargs=model_kwargs,
                    )
            else:
                out["mean"] = self.condition_mean(
                    cond_fn, out['mean'], t, model_kwargs=model_kwargs, 
    ...
    def p_sample_loop_progressive(
        ...
        for i in indices:
            ...
            with th.no_grad():
                ...
                cond_fn_with_grad = False  # NOTE: for eval debugging  #####   big bug  during debug, search it for long time!!1
    ...
    def ddim_sample_with_grad(self,
        model,
        x,
        t,
        clip_denoised=True,
    ...
    def masked_ce(self, a, b, mask, pos_weight=1.5):
        n_all = torch.numel(a)
        n_p = a.sum()
        # if n_p == 0:
        ...
        occ_loss = F.binary_cross_entropy_with_logits(b, a.float(), reduction='none', pos_weight=w_for_1)
        occ_loss = sum_flat(occ_loss * mask.float())  # gives \sigma_euclidean over unmasked elements
        n_entries = a.shape[1] * a.shape[2]
        non_zero_elements = sum_flat(mask) * n_entries
        occ_loss_val = occ_loss / non_zero_elements
        return occ_loss_val * 0.1

    def contact_loss(self, motion, contact_lable, mask):
        if self.std.device != motion.device:
            self.std = self.std.to(motion.device)
            self.mean = self.mean.to(motion.device)
        motion = motion * self.std + self.mean

        h_motion = motion[:, 0:263].permute(0, 2, 3, 1)
        o_motion = motion[:, 263:269].permute(0, 2, 3, 1)

        h_motion = recover_from_ric(h_motion.float(), 22)
        h_motion = h_motion.view(-1, *h_motion.shape[2:]).permute(0, 2, 3, 1)

        num_frames = h_motion.shape[-1]
        batch_size = h_motion.shape[0]
        num_frames = num_frames * batch_size
        if self.j2s is None:
            self.j2s = joints2smpl(num_frames=num_frames, device_id=str(h_motion.device).split(':')[1], cuda=h_motion.is_cuda)

        h_motion = h_motion.permute(0, 3, 1, 2).contiguous().view(-1, 22, 3)
        motion_tensor, opt_dict = self.j2s.joint2smpl_cuda(h_motion)  # [nframes, njoints, 3]
        n_all = torch.numel(a)
        n_p = a.sum()
        # if n_p == 0:
        #     print('target: no valid voxel when computing loss')
...
class LocalMotionDiffusion(GaussianDiffusion):
    ...
    def training_losses(self, model, mask, t_bar, *args, **kwargs):
...
class AffordDiffusion(GaussianDiffusion):
    ...
    def training_losses(self, model, mask, t_bar, *args, **kwargs):
        ...
        loss_mse_pos = mean_flat((items['target'][:,:3] - items['pred'][:,:3]) ** 2).view(-1, 1).mean(-1) # mean_flat(rot_mse)
        loss_mse_prob = mean_flat((items['target'][:,3:] - items['pred'][:,3:]) ** 2).view(-1, 1).mean(-1) # mean_flat(rot_mse)

        losses["loss"] =  loss_mse_pos + loss_mse_prob
        return losses
