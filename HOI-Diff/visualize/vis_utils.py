...
class npy2obj:
    def __init__(
        self, npy_path, sample_idx, rep_idx, device=0, cuda=True, if_color=False
    ):
        ...
        if self.nfeats == 3:
            print(
                f"Running SMPLify For sample [{sample_idx}], repetition [{rep_idx}], it may take a few minutes."
            )
            motion_tensor, opt_dict = self.j2s.joint2smpl(
                self.motions["motion"][self.absl_idx].transpose(2, 0, 1)
            )  # [nframes, njoints, 3]
        ...
        self.vertices = self.rot2xyz(
            torch.tensor(self.motions["motion"]),
            mask=None,
            pose_rep="rot6d",
            translation=True,
            glob=True,
        ...
        if if_color and "h_contact" in self.motions.keys():
            ...
            self.contact_idxs = np.zeros(
                (self.bs, self.vertices.shape[1], self.nframes), dtype=np.int8
            )
            self.contact_idxs[:, choose, :] = self.motions["h_contact"][
                self.absl_idx
            ].astype(np.int8)

            self.colors = np.zeros(
                (self.bs, self.vertices.shape[1], 4, self.nframes), dtype=np.int8
            )
    ...
    def save_npy(self, save_path):
        ...
        data_dict = {
            ...
            "root_translation": self.motions["motion"][
                0, -1, :3, : self.real_num_frames
            ],
            "faces": self.faces,
...
class npy2obj_object:
    def __init__(
        ...
        if len(self.motions["obj_name"][0].split("_")) > 2:
            obj_name = [b.split("_")[1] for b in self.motions["obj_name"]]
        else:
            obj_name = [b for b in self.motions["obj_name"]]
        self.motions["motion_obj"] = self.motions["motion_obj"]
        self.vertices, self.faces = self.pose2mesh(
            self.motions["motion_obj"], obj_path, obj_name
        )
        # self.j2s = joints2smpl(num_frames=self.num_frames, device_id=device, cuda=cuda)
        ...
        if if_color and "o_contact" in self.motions.keys():
            ...
            obj_sample_path = "/work/vig/yimingx/behave_obj_sample/{}.npy".format(
                self.motions["obj_name"][self.absl_idx]
            )
            choose = np.load(obj_sample_path)

            self.contact_idxs = np.zeros(
                (self.bs, self.vertices[self.absl_idx].shape[0], self.nframes),
                dtype=np.int8,
            )
            self.contact_idxs[:, choose, :] = self.motions["o_contact"][
                self.absl_idx
            ].astype(np.int8)

            self.colors = np.zeros(
                (self.bs, self.vertices[self.absl_idx].shape[0], 4, self.nframes),
                dtype=np.int8,
            )
    ...
    def pose2mesh(self, motion_obj, obj_path, obj_name):
        ...
        for b in range(self.bs):
            mesh_path = os.path.join(obj_path, obj_name[b], obj_name[b] + ".ply")
    ...
    def save_npy(self, save_path):
        data_dict = {
            "motion": self.motions["motion_obj"][
                self.absl_idx, :, :, : self.real_num_frames
            ],
            "faces": np.array(self.faces[self.absl_idx]),
            "vertices": np.array(
                self.vertices[self.absl_idx][:, :, : self.real_num_frames]
            ),
            # 'contact_idx': self.contact_idxs[0, :, :self.real_num_frames],
            "contact_idx": None,
