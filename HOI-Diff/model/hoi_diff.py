    ...
    def __init__(
        ...
        **kargs,
        ...
        if self.arch == "trans_enc":
            ...
            self.seqTransEncoder_start = nn.TransformerEncoder(
                seqTransEncoderLayer, num_layers=self.args.multi_backbone_split
            )
            self.seqTransEncoder_end = nn.TransformerEncoder(
        ...
        seqTransEncoderLayer_obj_pose = nn.TransformerEncoderLayer(
            d_model=self.latent_dim,
            nhead=self.num_heads,
            dim_feedforward=self.ff_size,
            dropout=self.dropout,
...
class MutualAttention(nn.Module):
    def __init__(self, num_layers, latent_dim, input_feats):
        ...
        seqTransDecoderLayer_obj = nn.TransformerDecoderLayer(
            d_model=self.latent_dim,
            nhead=self.num_heads,
            dim_feedforward=self.ff_size,
            dropout=self.dropout,
        ...
        seqTransDecoderLayer_human = nn.TransformerDecoderLayer(
            d_model=self.latent_dim,
            nhead=self.num_heads,
            dim_feedforward=self.ff_size,
            dropout=self.dropout,
        ...
        self.seqTransDecoder_human_pose = nn.TransformerDecoder(
            seqTransDecoderLayer_human, num_layers=self.num_layers
        )

        self.seqTransDecoder_obj_pose = nn.TransformerDecoder(
            seqTransDecoderLayer_obj, num_layers=self.num_layers
        )
