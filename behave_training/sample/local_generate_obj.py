...
def main():
    ...
    if is_using_data:
        ...
        if is_t2m:
            ...
            collate_args = [
                dict(
                    arg, text=txt, obj_points=points, obj_normals=normals, seq_name=name
                )
                for arg, txt, points, normals, name in zip(
                    collate_args, texts, obj_points, obj_normals, obj_name
                )
            ]
    ...
    # input to cuda
    model_kwargs["y"]["obj_points"] = model_kwargs["y"]["obj_points"].to(
        dist_util.dev()
    )
    model_kwargs["y"]["obj_normals"] = model_kwargs["y"]["obj_normals"].to(
        dist_util.dev()
    )
    ...
    for rep_i in range(args.num_repetitions):
        ...
        input_dim = motion_model.njoints + 6 + 2 * HAND_FEATURE_DIM
        print(f"input dim: {input_dim}")
        print(f"motion model njoints: {motion_model.njoints}")
        print("--------------------------------------------")
        sample = sample_fn(
            motion_model,
            (
        ...
        # sample shape: [num_samples, 1, seq_len, motion_dim]

        # get object sample
        sample_obj = sample[..., 263:269]  # [num_samples, 1, seq_len, 6]
        sample_obj = sample_obj.permute(0, 1, 3, 2)  # [num_samples, 1, 6, seq_len]
        ...
        all_motions_lhand.append(lhand_sample.cpu().numpy())
        all_motions_rhand.append(rhand_sample.cpu().numpy())
    ...
    all_motions_lhand = np.concatenate(all_motions_lhand, axis=0)[:total_num_samples]
    all_motions_rhand = np.concatenate(all_motions_rhand, axis=0)[:total_num_samples]
    ...
    save_dict = {
        "motion": all_motions,
        "motion_obj": all_motions_obj,
        "motion_lhand": all_motions_lhand,
        "motion_rhand": all_motions_rhand,
        "text": all_text,
        "lengths": all_lengths,
    ...
    for sample_i in range(args.num_samples):
        ...
        for rep_i in range(args.num_repetitions):
            ...
            # these all motions arrays have the shape [num_samples, 1, motion_dim, seq_len]
            motion_obj = all_motions_obj[rep_i * args.batch_size + sample_i].transpose(
                2, 0, 1
            )[:length, 0, :]

            motion_lhand = all_motions_lhand[
                rep_i * args.batch_size + sample_i
            ].transpose(2, 0, 1)[:length, 0, :]

            motion_rhand = all_motions_rhand[
                rep_i * args.batch_size + sample_i
            ].transpose(2, 0, 1)[:length, 0, :]

            # hand vector shapes: [seq_len, 30]

            vertices = all_obj_points[rep_i * args.batch_size + sample_i]

            if is_using_data:
                obj_name = all_obj_name[rep_i * args.batch_size + sample_i].split("_")[
                    1
                ]
            ...
            mesh_path = os.path.join(
                "./dataset/behave_t2m/object_mesh", obj_name, obj_name + ".ply"
            )
            ...
            # transform
            angle, trans = (
                motion_obj[:, :3].transpose(1, 0),
                motion_obj[:, 3:].transpose(1, 0),
            )
            rot = Rotation.from_rotvec(angle.transpose(1, 0)).as_matrix()
            obj_points = (
                np.matmul(vertices[np.newaxis], rot.transpose(0, 2, 1)[:, np.newaxis])[
                    :, 0
                ]
                + trans.transpose(1, 0)[:, np.newaxis]
            )
            ...
            lhand_joints, rhand_joints, lhand_wrists, rhand_wrists = (
                generate_hand_joints(
                    motion, motion_lhand, motion_rhand, hand_mode="joints"
                )
            )
            plot_3d_motion(
                animation_save_path,
