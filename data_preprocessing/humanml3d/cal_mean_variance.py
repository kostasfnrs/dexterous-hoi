...
def mean_variance(data_dir, save_dir, joints_num):
    ...
    Std[
        4 + (joints_num - 1) * 9 + joints_num * 3 : 4
        + (joints_num - 1) * 9
        + joints_num * 3
        + 4
    ] = (
    ...
    Std[
        4 + (joints_num - 1) * 9 + joints_num * 3 + 4 : 4
        + (joints_num - 1) * 9
        + joints_num * 3
        + 7
    ] = (
    ...
    Std[
        4 + (joints_num - 1) * 9 + joints_num * 3 + 7 : 4
        + (joints_num - 1) * 9
        + joints_num * 3
        + 10
    ] = (
    ...
    if rot_dim != 0:
        ...
        Std[start_dim : start_dim + rot_dim] = (
            Std[start_dim : start_dim + rot_dim].mean() / 1.0
        )
        start_dim += rot_dim

    # left hand
    # Std[start_dim:start_dim + hand_dim] = Std[start_dim:start_dim + hand_dim].mean() / 1.0
    # for numerical stability, since we will divide by the Std
    ...
    assert (
        8 + (joints_num - 1) * 9 + joints_num * 3 + 6 + 2 * hand_dim + 2 * rot_dim
        == Std.shape[-1]
    )
